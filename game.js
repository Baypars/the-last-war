...
function replay() {
    ...
    replay.addEventListener('click', function() {
        box.forEach(function(box) {
            ...
            box.style.backgroundImage = 'url("svg/")';
        });
        let bang = document.querySelector('.won');
        newFactory = setInterval(randomFactory, 700);
...
function addTree(e) {
    ...
    c.style.backgroundImage = 'url("svg/tree.svg")';
    console.log(arrTree);
}
...
function randomFactory() {
    ...
    if(arrFactory.indexOf(box[g].dataset.value) == -1) {
        ...
        box[g].style.backgroundImage = 'url("svg/factorina.svg")';
...
start.addEventListener('click', function() {
    ...
    init.style.top = '100%';
    newFactory = setInterval(randomFactory, 800);
});
